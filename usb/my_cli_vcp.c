...
#define USB_CDC_PKT_LEN CDC_EP_PACKET_SIZE
...
static void vcp_on_cdc_state_callback(void* context, uint8_t state);
static void vcp_on_cdc_control_line(void* context, uint8_t state);

static CdcCallbacks cdc_cb = {
    vcp_on_cdc_tx_complete,
    vcp_on_cdc_rx,
    vcp_on_cdc_state_callback,
...
static void cli_vcp_init(void) {
    ...
    vcp->thread = furi_thread_alloc_ex("MyCliVcpWorker", 1024, vcp_worker, NULL);
...
static int32_t vcp_worker(void* context) {
    ...
    if((vcp->usb_if_prev != &hid_with_cdc_intf)) {
        furi_hal_usb_set_config(&hid_with_cdc_intf, NULL);
    }
    composite_cdc_set_callbacks(&cdc_cb, NULL);
    ...
    while(1) {
        ...
        if(flags & VcpEvtRx) {
            if(furi_stream_buffer_spaces_available(vcp->rx_stream) >= USB_CDC_PKT_LEN) {
                int32_t len = composite_cdc_receive(vcp->data_buffer, USB_CDC_PKT_LEN);
                VCP_DEBUG("Rx %ld", len);
        ...
        if(flags & VcpEvtTx) {
            ...
            if(len > 0) { // Some data left in Tx buffer. Sending it now
                tx_idle = false;
                composite_cdc_send(vcp->data_buffer, len);
                last_tx_pkt_len = len;
        ...
        if(flags & VcpEvtStop) {
            vcp->connected = false;
            vcp->running = false;
            composite_cdc_set_callbacks(NULL, NULL);
...
static size_t cli_vcp_rx(uint8_t* buffer, size_t size, uint32_t timeout) {
    ...
    if(vcp->running == false) {
        VCP_DEBUG("rx: vcp is not running");
        return 0;
    }
...
static void cli_vcp_tx(const uint8_t* buffer, size_t size) {
    ...
    if(vcp->running == false) {
        VCP_DEBUG("tx: vcp is not running");
        return;
    }
